...
def diagram(d,limits):
    ...
    if k=='stress':
        ...
        if k=='influence':
            ...
            line((-2.4,-.15),(2.4,-.15),GREY);g.add(txt('0',15,GREY).move_to([-2.55,-.15,0]),txt('+',19,BLUE).move_to([-1.1,.45,0]),txt('−',19,ORANGE).move_to([1.15,-.45,0]))
            pts=d['points'];s=1.3/max(abs(y) for _,y in pts);g.add(poly([(-2.25+4.5*x,-.15+y*s) for x,y in pts],BLUE))
            if d.get('negative'):
