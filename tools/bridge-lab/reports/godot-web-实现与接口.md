...
Web 请求统一设置 `accept_gzip = false`，由浏览器解压 gzip/deflate；原生引擎保持该值为 true。Godot 4.7 Web Fetch 返回已解压的数据，但保留 Content-Encoding，默认 HTTPRequest 会错误地二次解压。Web 的响应长度由引擎固定返回 -1，进度使用清单中的解码后字节数，不能将 CDN 的压缩 Content-Length 当作解码数据总量。详见 `godot-gzip-fix.md`。
