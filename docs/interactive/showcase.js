...
(() => {
 ...
 function chooseVideo(v,play=false){
  currentVideo=v;player.pause();player.replaceChildren();player.poster=v.poster;player.src=v.src;
  ...
  player.load();renderVideos();if(play)player.play().catch(()=>{});
 }
 ...
 function renderImages(){
  ...
  $('#image-grid').innerHTML=all.slice(0,imageLimit).map(x=>`<article class="image-card"><button data-id="${x.id}" aria-label="放大：${esc(x.title)}"><img src="${esc(x.src)}" alt="${esc(x.title)}" loading="lazy">${imageKind==='ai'?`<span class="badge ${x.revision?'warning':''}">${x.revision?'构造待修正':x.selected?'已选 · 正文试排':'备选画法'}</span>`:''}</button><div class="image-copy"><small>${x.id}</small><h3>${esc(x.title)}</h3>${imageKind==='ai'?`<p>${esc(x.note)}</p>`:''}${x.link?`<p><a href="${esc(x.link)}">在教材中阅读 ↗</a></p>`:''}</div></article>`).join('');
